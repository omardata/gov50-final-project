...
In this paper I will data relating voting patterns in international organizations to the level of aid that a country receives from the USA. I will look at how well a state's level of aid from the USA can explain that state's level of voting coincidence with the US in the United Nations General Assembly. 

Personal question: Is it voting coincidence, or is it state preference? What is the outcome variable? Might be able to use both...   
My  research question is:  Are the recepients of US aid more inclined to US-aligned voting in the UNGA? Does this aid change their state preference. This question is interesting because ___. 

My central hypothesis is that recipients of US aid vote more frequently in line with the US.
...
This is interesting, as a question because____. 
This is interesting, as a hypothesis because ___.
...
My dependent variable, `distance`,is calculated from data of estimated measures of state preference as provided by Dreher et. all 2008. Dreher et all provide a numeric measure of the variable `idealpointestimate`  



state/session level measure of state preference as calculated from United Nations General Assembly Voting Data. For a given session, the values of these sets of the  


measure of state preference as calculated from United Nations General Assembly Voting Data.reher et. all 2008.




who provides a measure of state preference as calculated from United Nations General Assembly Voting Data.reher et. all 2008.

The variable that I am using can be calculated from the ideal point estimations that are given by ___. This is a measure of the difference between the USA ideal point and a country's ideal point, for a given session. I have named this variable `distance` and it is what we are trying to explain, for a given country in a given year. The value for the US in a given year is zero, because the US displayed voting patterns identical to itself. Therefore a distance of zero means relatively similar voting patterns?  

Data for my independent variable is taken from AID, which provides aggregated data on the amount of bilateral aid that a country reports that they committed to giving another country. 
