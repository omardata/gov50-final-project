...
```{r}
ideal_aid|> filter(!is.na(commitment_amount_usd_constant_sum))|>
  ggplot(mapping = aes(x = commitment_amount_usd_constant_sum,
                       y = distance_abs))+
  geom_point()+

fit <- lm(distance_abs ~ commitment_amount_usd_constant_sum, data = ideal_aid)
