...
In summary my findings are that on average the commitment of any amount of bilateral aid from the US to another country is correlated with a foreign-policy preference convergence between the US and that country.

My first and primary analysis is simply taking any commitment of aid as treatment. Using a difference in differences approach for the average chang from the prvious year to the current year, we find that a country in the treatment group is on average predicted to have a decrease in their distance from the United States. The regression aanlysis on the treatment as a categorical predictor yields two coefficients. One is an intercept which represents the models prediction for the baseline group, that is the control. The value of the intercept is positive at __ indicating that on average countries in the control were slighly more likely to increase their distancce from the US. However it is imporant to keep in mind that For countries in the control this model is rather uninformative, this intercept coeficient is not statistically signifcant, and in fact is highly arebitrary. For countries in the control this model is rather uninformative. 
...
##Regression on data in ATE table 
ate_fit <- lm(change_one_before ~ treatment, data = ate_table_longer)



##Distribution Chart for all observered country changes
ideal_treat_fil|> 
